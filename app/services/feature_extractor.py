...
_REQUEST_TIMEOUT = 5  # seconds — per-call limit; calls run in parallel
...
def _global_rank(domain: str) -> int:
    """Look up global traffic rank via Alexa/TRANCO-style DNS TXT record.

    Returns a positive integer rank on success, or 0 on failure.
    0 is used as a sentinel so callers can distinguish "known low-traffic"
    from "lookup failed" — preventing failures from being misread as phishing.
    """
    try:
        # TRANCO list lookup — free, no API key required
        resp = requests.get(
...
def _redirect(response: requests.Response | None) -> int:
    ...
    if history_len <= 1:
        return 1   # 0-1 redirects → legitimate
    if history_len <= 4:
        return 0   # 2-4 redirects → suspicious
    return -1      # 5+ redirects → phishing
...
def _dns_record(whois_data: Any) -> int:
    """Reuses already-fetched whois data — avoids a second blocking whois call."""
    return 1 if whois_data is not None else -1


def _web_traffic(rank: int) -> int:
    # rank == 0 means lookup failed — use 0 (neutral) not -1 (phishing)
    if rank == 0:
        return 0
...
def _page_rank(rank: int) -> int:
    ...
    if rank == 0:
        return 0
    return 1 if rank < 10_000 else -1


def _google_index(url: str) -> int:
    """Check Google index via a direct URL fetch rather than the scraping library.

    googlesearch-python is not a declared dependency and gets rate-limited.
    Instead, check if the site's sitemap or robots.txt is reachable as a proxy
    for being a real, indexed site.
    """
    try:
        ...
        return 0  # unknown — not a phishing signal
...
# Canonical source of truth for feature names in extraction order — shared by
# training, inference, and metadata.json so ordering never drifts.
FEATURE_LIST: list[str] = FEATURE_COLUMNS


def extract(url: str) -> dict[str, float]:
    """Top-level inference interface: return features as a named dict.

    All callers (detection_service, training scripts) must use this function.
    Never duplicate feature logic elsewhere.

    NOTE: values are negated before returning.  The individual feature helpers
    use the original UCI convention (1 = legitimate, -1 = phishing) which is
    intuitive to read.  The training dataset uses the opposite encoding
    (1 = phishing indicator, -1 = legitimate indicator) for 22/30 features.
    Negating here reconciles the two so the model receives values that match
    the distribution it was trained on — without having to rewrite every helper.
    0 (uncertain/unknown) negates to 0 and stays neutral.
    """
    vector, _ = extract_features(url)
    return {name: -float(value) for name, value in zip(FEATURE_COLUMNS, vector)}


def validate_feature_vector(features: dict) -> bool:
    """Return True iff every expected feature is present and numeric."""
    return all(
        name in features and isinstance(features[name], (int, float))
...
def extract_features(url: str) -> tuple[list[int], int]:
    ...
    # Run the three slow network calls in parallel — cuts latency from ~sum to ~max
    with ThreadPoolExecutor(max_workers=3) as pool:
        fut_fetch = pool.submit(_fetch, url)
        fut_whois = pool.submit(_whois_safe, domain)
        fut_rank  = pool.submit(_global_rank, domain)
        response, soup = fut_fetch.result()
        whois_data      = fut_whois.result()
        rank            = fut_rank.result()
